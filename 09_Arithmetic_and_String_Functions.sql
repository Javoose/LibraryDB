...
SELECT Title, (PageCount * StockQuantity) AS [Total Pages in Stock]
FROM Books;

-- 2. Calculate the stock value of each book if every copy is valued at 50
SELECT Title, (50 * StockQuantity) AS [Stock Value]
FROM Books;

-- 3. Calculate half of the page count of each book
SELECT Title, (PageCount / 2.0) AS [Half Page Count]
FROM Books;
...
SELECT Title, ROUND(SQRT(PageCount),0) AS [Square Root of Page Count]
FROM Books;

-- 3. Calculate the square of each book's stock quantity
SELECT Title, POWER(StockQuantity,2) AS [Stock Quantity Squared]
FROM Books;
...
SELECT Title, LEN(Title) AS [Number of Characters]
FROM Books;
...
SELECT Country, REPLACE(Country,'United','UK') AS [Modified Country]
FROM Authors;

-- CHARINDEX AND REVERSE PRACTICE

-- 1. Find the position of the letter 'a' in each book title
SELECT Title, CHARINDEX('a',Title) AS [Position of 'a']
FROM Books;

-- 2. Display each book title in reverse order
SELECT Title, REVERSE(Title) AS [Reversed Title]
FROM Books;
